...
func (s *Subscription) FindOne(
	...
	return s.findOne(ctx, resourceID, id)
}
...
func (s *Subscription) Trigger(
	...
	subscription, err := s.findOne(ctx, rawDocument.Resource.ID, rawSubscription.ID)
	if err != nil {
		if repoErr, ok := err.(flare.SubscriptionRepositoryError); ok {
			if repoErr.NotFound() {
				return nil
			}
		}
	...
	doc, err := s.documentRepository.FindOne(ctx, rawDocument.ID)
	if err != nil {
		if repoErr, ok := err.(flare.DocumentRepositoryError); ok {
			if repoErr.NotFound() {
				return nil
			}
		}
	...
	reference, err := s.documentRepository.FindOne(ctx, doc.ID)
	if err != nil {
		return errors.Wrap(
			err,
