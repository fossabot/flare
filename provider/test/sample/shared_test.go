...
func initMongoDBResourceRepositoriy(ip string) (flare.ResourceRepositorier, func(), error) {
	mongodbClient, err := mongodb.NewClient(mongodb.ClientAddrs([]string{ip}))
	if err != nil {
		return nil, nil, err
	}

	var (
		mongodbResource     mongodb.Resource
		mongodbSubscription mongodb.Subscription
	)

	err = mongodbResource.Init(
		mongodb.ResourceClient(mongodbClient),
		mongodb.ResourceSubscriptionRepository(&mongodbSubscription),
	)
	if err != nil {
		return nil, nil, err
	}

	err = mongodbSubscription.Init(
		mongodb.SubscriptionClient(mongodbClient),
		mongodb.SubscriptionResourceRepository(&mongodbResource),
	)
	if err != nil {
		return nil, nil, err
	}

	return &mongodbResource, mongodbClient.Stop, nil
}
